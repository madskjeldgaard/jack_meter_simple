...
static int usage(const char *progname) {
  ...
  fprintf(stderr, "       -s      is the [optional] name given the jack server "
                  "when it was started\n");
  fprintf(stderr, "       -n      changes mode to output meter level as number "
                  "in decibels\n");
  fprintf(stderr,
...
int main(int argc, char *argv[]) {
  ...
  if (!(input_port = jack_port_register(client, "in", JACK_DEFAULT_AUDIO_TYPE,
                                        JackPortIsInput, 0))) {
